...
function Header()
{
 ...
 $this->SetFillColor(45, 87, 156);
   ...
   $this->Cell(120,80,utf8_decode('Reporte de deserción de alumnos '),0,0,'C');
   ...
   //color de la lineas
    ...
    $this->Cell(50,10,utf8_decode('FACULTAD'),0,0,'C',1);
    $this->Cell(30,10,utf8_decode('FECHA'),0,0,'C',1);
    $this->Cell(30,10,'DESCRIPCIÓN',0,1,'C',1);
...
$pdf->setTitle('Reporte de deserción de alumnos');
//color de fila
...
$cont=1;
      ...
      $sql = $conexion -> query("SELECT concat(p.nombre,' ',p.apellido)datos,f.nombre as facultad, da.fecha,id.descripcion  FROM persona p
      INNER JOIN alumno a on p.id=a.persona_id
      INNER JOIN escuela e on a.escuela_id=e.id
      INNER JOIN facultad f on f.id=e.facultad_id
      INNER JOIN desercion_alumno da on da.alumno_id=a.id
      INNER JOIN indices_de_desercion id on id.id=da.desercion_id");
      ...
      foreach($sql->fetchAll() as $empleados){
            ...
            $pdf->Cell(50,10,($empleados['facultad']),1,0,'C',1);
            $pdf->Cell(30,10,($empleados['fecha']),1,0,'C',1);
            $pdf->Cell(30,10,($empleados['descripcion']),1,1,'C',1);
